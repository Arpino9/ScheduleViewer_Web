...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pane-schedule').addEventListener('click', e => {
    ...
    if (boxBtn) openBoxAttachModal(boxBtn.dataset.eventId, boxBtn.dataset.eventTitle);
...
function renderEventCard(e) {
  ...
  const attachBtn = e.eventId
    ? `<button class="box-attach-btn" data-event-id="${esc(e.eventId)}" data-event-title="${esc(e.title || e.displayTitle || '')}">📎 Box添付</button>` +
      `<button class="photo-attach-btn" data-event-id="${esc(e.eventId)}" data-event-title="${esc(e.title || e.displayTitle || '')}">📷 写真</button>`
    : '';
  return `
...
async function waitForCalendarReload() {
  ...
  for (let i = 0; i < 60; i++) {
    const loading = await fetch('/api/calendar/status').then(r => r.json()).catch(() => false);
    if (!loading) return;
...
async function submitPhotoAttach(e) {
  e.preventDefault();
  ...
  try {
    const res = await fetch(`/api/calendar/events/${encodeURIComponent(_photoAttachEventId)}/photo`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ photoUrl: document.getElementById('photo-attach-url').value })
    });
    if (!res.ok) throw new Error('HTTP ' + res.status);
